...
\title{Recovering Latent Age Distributions from Misclassified Observations: A Simulation Study Using EM and Method of Moments}
...
\paragraph{AI usage}
ChatGPT has been used as a tool for language refinement, structuring suggestions, and clarification of statistical concepts. All scientific content, modelling choices are my own.

\tableofcontents
\section{Introduction}
Accurate estimates of population size are of central importance in ecology and wildlife management, as they provide the basis for assessing population resilience, conservation status, and long-term viability. For large carnivores in particular, reliable population estimates are essential for informing management decisions, including conservation planning and hunting regulation \parencites{https://doi.org/10.2981/10-100}.

Although current monitoring programs provide estimates of population size and sex distribution, reliable estimation of age structure remains more challenging. In Sweden, the brown bear population has been continuously monitored by the Swedish Museum of Natural History, which has served as the national coordinator for bear inventories on behalf of the Swedish Environmental Protection Agency since 2018. Within this framework, population size and sex distributions are estimated using DNA sampling from faeces combined with capture--recapture methods \parencite{asbrink2025, asbrink2023_bjorninventering}.

While total population size is fundamental, understanding the age composition of a population is also essential because demographic processes such as survival and reproduction vary strongly with age \parencite{article}. The importance of age-structured population data has recently been emphasized in a synthesis report summarizing 40 years of Scandinavian brown bear research \parencite{naturvardsverket2025}, which identified the need for further research on age structure in the Swedish brown bear population.

In bear populations, age typically cannot be measured directly. A common approach involves analyzing tooth cementum layers, corresponding to annual growth rings in teeth. However, this method is invasive and requires the removal of a molar. As a recent development, DNA-based age estimation methods have been proposed \parencite{nakamura2023}. These indirect measurements are typically associated with measurement error. According to \cite{https://doi.org/10.1890/07-0369.1}, researchers often respond to substantial measurement error either by excluding noisy data or by ignoring the measurement error entirely.
...
The present thesis investigates this deconvolution problem in a controlled simulation setting. The true population age structure is generated using a Leslie matrix model parameterized with data from \parencite{nina_bearmodel}. Measurement error is then introduced through a probabilistic misclassification mechanism, producing an observed age distribution that deviates from the latent structure.

The primary objective of this study is to evaluate how accurately the latent age distribution can be recovered using deconvolution approaches based on the Expectation--Maximization (EM) algorithm and the method-of-moments (MoM) estimator. The misclassification mechanism  was parameterized to resemble the error structure reported in \parencite{nakamura2023}, while an additional high-measurement-error regime was included to evaluate performance under more severely ill-posed conditions. Using Monte Carlo simulations, the estimators were evaluated across five different sample sizes. Recovery performance was assessed using total variation distance, while the ecological implications of estimation error were examined through population forecasting based on the Leslie matrix model.
...
\begin{table}[H]
	...
	\begin{tabular}{ll}
		...
		$n$ & Sample size \\
		$T_r$ & True age class for individual $r$ \\
		$O_r$ & Observed age class for individual $r$ \\
		$\boldsymbol{\pi}$ & Latent age distribution \\
		$\mathbf{Z}$ & Vector of true age-class counts \\
		$\mathbf{Y}$ & Vector of observed age-class counts \\
		$M_{ji}$ & Probability of observing class $i$ given true class $j$ \\
		$\mathbf{M}$ & Misclassification matrix \\
		$\hat{\boldsymbol{\pi}}_{\text{naive}}$ & Naive estimator \\
		$\hat{\boldsymbol{\pi}}_{\text{MoM}}$ & Method-of-moments estimator \\
		$\hat{\boldsymbol{\pi}}_{\text{ML}}$ & EM-based maximum likelihood estimator \\
		$\mathbf{L}$ & Leslie matrix \\
		$\lambda_1$ & Dominant eigenvalue of $\mathbf{L}$ \\
		$\mathbf{w}$ & Stable age distribution eigenvector \\
...
This section focuses on the general theoretical framework and its adaptation to the present setting, while detailed parameter choices and simulation specifications are deferred to Section \ref{sec:simulation_study} unless needed to motivate specific modeling choices. We begin by introducing the general deconvolution framework and then specialize it to the present setting of age misclassification in discrete populations. In the subsequent section on estimation methods, we present the two estimation approaches considered in this thesis: a method-of-moments estimator and a likelihood-based estimator based on the Expectation--Maximization algorithm, including a full derivation of the EM procedure. We then introduce matrix population models, which are used both to construct a biologically grounded population age distribution and to generate population forecasts error under estimated and true age structures. Finally, we define the evaluation metric used throughout the study, the total variation distance, which quantifies discrepancies between true and estimated age distributions.

\subsection{Deconvolution}\label{sec:deconvolution}

This study considers a deconvolution problem in which the objective is to recover the distribution of a latent variable \(Z\) from observations of an error-contaminated variable \(Y\). This can be formulated through a convolution structure in which the observed variable is a noisy transformation of the true signal \parencite[Chapter 12]{carroll2006}.

\[
Y = Z + \epsilon.
\]
Here, $Z$ denotes the latent random variable, $\epsilon$ denotes measurement error, and $Y$ denotes the observed random variable. The distribution of $Y$ is then the convolution of the distributions of $Z$ and $\epsilon$. This formulation extends naturally to multivariate settings, where both the latent and observed quantities are represented as random vectors.
...
\[
\boldsymbol{Z} = \left(Z_1, Z_2, \dots, Z_k\right),
\]
where

\[
Z_j = \sum_{r=1}^{n} \boldsymbol{1}(T_r = j)
\]
...
In this study the measurement error is assumed to be fully known. Moreover, we assume a normal distribution for the error term as a simple and tractable way to control the variability in the observations and to introduce systematic measurement error. While this assumption may not fully reflect the error structure described in \cite{nakamura2023}, the aim of this thesis is to study the recovery of the latent distribution under a simplified but comparable error setting.
...
\]
Applying the measurement error directly at the individual level would yield a continuous latent measurement $T_r + \epsilon_r$, which is incompatible with the discrete age-class structure of the model. We therefore define a misclassification induced by this continuous error model by discretizing the conditional distribution of $\epsilon_r \mid T_r = j$ over the $k$ predefined age classes. Let the probabilities $M_{ji}$ denote the probability of misclassifying a bear of true age j with age i:
...
\sum_{i = 1}^{k} M_{ji} = 1.
\]
This leads to the construction of a square misclassification matrix $\boldsymbol{M}$ with $k$ rows and columns, where row $j$ corresponds to the true age class and column $i$ to the observed age class.

\paragraph{Observed data} \label{par:observed_dist}

Given the misclassification matrix $\boldsymbol{M}$ described above, each individual with true age class $j$ is observed in age class $i$ with probability $M_{ji}$. The observed count vector $\mathbf{Y} = (Y_1, \dots, Y_k)$ therefore follows a multinomial distribution with probability vector $\boldsymbol{M}\boldsymbol{\pi}$:
\begin{equation}
	\mathbf{Y} \sim \text{Multinomial}(n, \boldsymbol{\pi} \boldsymbol{M}),
	\label{eq:observed_distribution}
\end{equation}
where 
\[
Y_i = \sum_{r=1}^{n} \boldsymbol{1}(O_r = i).
\]

\subsection{Estimation methods}

In this thesis, we consider two methods to recover the latent age structure $\boldsymbol{\pi}$: a method-of-moments (MoM) estimator and a maximum likelihood estimator based on the EM algorithm. We define an oracle baseline based on $\boldsymbol{Z}$ and a naive estimator based on $\boldsymbol{Y}$ as reference points.

\subsubsection{Oracle and naive estimator}

In the absence of measurement error and with access to the true age counts, a natural benchmark estimator is given by the empirical proportions of the true age classes:
\begin{equation*}
	\hat{\boldsymbol{\pi}}_{\text{oracle}} = \frac{\mathbf{Z}}{n}.
	\label{eq:oracle_estimate}
\end{equation*}
We refer to this as the oracle estimator, since it assumes access to the unobserved true counts $\mathbf{Z}$. It serves as a benchmark representing the best-case performance given access to the true age counts. In contrast, when only misclassified observations are available and measurement error is ignored, a natural baseline estimator is given by the empirical proportions of the observed data:
\begin{equation}
	\hat{\boldsymbol{\pi}}_{\text{naive}} = \frac{\mathbf{Y}}{n}.
	\label{eq:naive_estimate}
\end{equation}
The naive estimator uses the observed data $\mathbf{Y}$ without correcting for measurement error. It therefore serves as a baseline for assessing whether the proposed deconvolution methods improve estimation of $\boldsymbol{\pi}$.


\subsubsection{Method of Moments estimator}

The method-of-moments assumes that moments, such as $\mathbb{E}[\mathbf{Z}]$, can be expressed as functions of the unknown parameter $\boldsymbol{\pi}$. Parameter estimates are obtained by equating these theoretical moments with their empirical counterparts, see \cite[p.~275]{AlmBritton2008}. 

From equation~\eqref{eq:observed_distribution}, the first moment of the observed data is given by
\begin{equation*}
	\mathbb{E}[\boldsymbol{Y}] = n  \boldsymbol{\pi} \boldsymbol{M}.
\end{equation*}
The corresponding empirical moment is given by the observed proportions $\boldsymbol{Y}/n$ see equation~\eqref{eq:naive_estimate}. Equating theoretical and empirical moments yields
\begin{equation*}
	 \boldsymbol{\pi} \boldsymbol{M} = \frac{\boldsymbol{Y}}{n}.
\end{equation*}
Solving for $\boldsymbol{\pi}$ gives the MoM estimator
\begin{equation*}
	\hat{\boldsymbol{\pi}}_{MoM} =  \frac{\boldsymbol{Y}}{n}\boldsymbol{M^{-1}},
\end{equation*}
provided that $\boldsymbol{M}$ is invertible.



\paragraph{Matrix regularization}

In practice, the misclassification matrix $\boldsymbol{M}$ may be nearly singular, which makes direct inversion numerically unstable. To address this issue, we apply Tikhonov regularization (ridge regularization), which stabilizes the inversion by penalizing large fluctuations in the estimated parameter vector.

The regularized estimator is defined as
\begin{equation*}
	\hat{\boldsymbol{\pi}} =
	\left(\boldsymbol{M}^\top \boldsymbol{M} + \lambda \boldsymbol{I}\right)^{-1} \boldsymbol{M}^\top \frac{\mathbf{Y}}{n},
\end{equation*}
...
\subsubsection{Maximum likelihood estimation via the EM algorithm} \label{sec:ML}

The EM algorithm is a general-purpose method for computing maximum likelihood estimates in the presence of incomplete or latent data \parencite{mclachlan2008}. Let $\boldsymbol{Y}$ denote a discrete random vector corresponding to the observed (incomplete) data, and $\boldsymbol{Z} $ a discrete random vector representing the latent data. Then $\boldsymbol{X}=(\boldsymbol{Y},\boldsymbol{Z})$ is the random vector of the complete data. If $\mathbf{X}$ were fully observable, we could directly compute the complete-data log-likelihood function $\ell_c(\boldsymbol{\pi})$ based on a realization $x$ and estimate $\boldsymbol{\pi}$ by maximum likelihood.
...
\begin{equation}
	...
	\label{eq:arg_max}
\end{equation}
The process is then iterated upon until $\ell(\mathbf{\pi}^{(t+1)}) - \ell(\mathbf{\pi}^{(t)}) \leq  \epsilon$ for some arbitrary small $\epsilon$ value, which is then interpreted as a local/global maxima.
...
\paragraph{Applying EM-algorithm}

Let $\boldsymbol{X} = {X_1, X_2, \ldots, X_n}$ denote the complete data which can be represented by individual-level pairs $X_r = (O_r,T_r)$. Since, in the present multinomial setting, it is convenient to use the sufficient statistic $N_{ji}$, where $N_{ji}$ denotes the number of individuals with true age class $j$ that are observed in age class $i$. 

The joint distribution for a single individual of true age class $T$ and observed age $O$ classes is:
\begin{equation*}
	P(T=j, O=i) = (\pi_j M_{ji}),
\end{equation*}
where $\boldsymbol{M}$ is the misclassification matrix defined in \ref{par:measurement-error} and for $n$ independent observations  $\boldsymbol{X}$ we obtain:

\begin{equation*}
	p_X(\boldsymbol{X};\boldsymbol{\pi}) = \prod_i \prod_j (\pi_j M_{ji})^{n_{ji}}.
\end{equation*}
The complete data log-likelihood is then: 

\begin{equation}
	\ell_c(\boldsymbol{\pi}) =  \sum_{j} \sum_{i} n_{ji} \log (\pi_j M_{ji}) = \sum_{j} \sum_{i} n_{ji} \log  M_{ji} + \sum_{j} \sum_{i} n_{ji} \log \pi_j.
	\label{eq:decomp_likelihood}
\end{equation}
From equation~\eqref{eq:Qfunction} we take the conditional expectation on $n_{ji}$ with an initial parameter $\boldsymbol{\pi^{(0)}}$ conditioned on $\boldsymbol{Y}$ and obtain:


\begin{align*}
	\mathbb{E}_{\boldsymbol{\pi}^{(0)}} \big[ \ell_c(\boldsymbol{\pi}) \mid \mathbf{Y} = \mathbf{y} \big]
	&=
	\mathbf{E}_{\boldsymbol{\pi}^{(0)}} \Big[ \sum_{j} \sum_{i} n_{ji} \log (\pi_j M_{ji}) \,\Big|\, \mathbf{Y} = \mathbf{y} \Big] \\
	&=
	\sum_{i} \sum_{j} \mathbf{E}_{\boldsymbol{\pi}^{(0)}} \big[n_{ji} \mid \mathbf{Y} = \mathbf{y}\big] \log (\pi_j M_{ji}).
\end{align*}
The conditional expectation $E_{\boldsymbol{\pi^{(0)}}}[n_{ji} \mid \mathbf{Y} = \mathbf{y}]$ is the expected number of bears with true age class $j$ that are observed in age class $i$, given the observed data $\mathbf{Y} = \mathbf{y}$. Note that this depends on the assumed latent age distribution $\boldsymbol{\pi^{(0)}}$.
...
\begin{equation}
P(T=j \mid O=i, \boldsymbol{\pi}^{(t)})
= \gamma_{ji}^{(t)} =
\frac{
	\pi_j^{(t)} M_{ji}
}{
	\sum_{a=1}^{k} \pi_a^{(t)} M_{ai}
}.
\end{equation}
Thus, the expected number of individuals with true age class $j$ among the $y_i$ individuals observed in age class $i$ is

\begin{equation}
	\mathbb{E}[n_{ji} \mid Y_i = y_i]
	=
	y_i \gamma_{ji}^{(t)}
	\label{eq:expected_nr_of_indi}
\end{equation}


\paragraph{Maximization-step}

We now choose $\boldsymbol{\pi}^{(t+1)}$ such that, in accordance with equation~\ref{eq:arg_max}, the conditional expected log-likelihood is maximized. From equation~\ref{eq:decomp_likelihood}, we note that only the terms involving $\boldsymbol{\pi}$ depend on the parameter. The problem is reduced to maximizing:
\[
\sum_{j} \mathbb{E}[n_{ji} \mid \mathbf{Y} = \mathbf{y}] \, \log \pi_j,
\]
...
\]
Using equation~\eqref{eq:expected_nr_of_indi}. The maximization step is then given by
\begin{equation}
	\pi_{j}^{(t+1)} =  \frac{\sum_{i=1}^k Y_i \gamma_{ji}^{(t)}}{n}.
	\label{eq:maximization_step}
\end{equation}


\subsection{Matrix population models}\label{sec:pop_model_matrix}

Matrix population models are a standard framework in population ecology \parencite{caswell2001}. In this section, we describe the theoretical framework used to construct a hypothetical true age distribution $\boldsymbol{\pi}$, which is used as the parameter of a multinomial model for simulating population data used to evaluate the deconvolution methods considered in this thesis.
...
A stable age distribution describes the long-term relative proportions of individuals in each age class of a population. It arises under the assumption of constant demographic parameters, meaning that both the survival probabilities $\mathbf{s}$ and fertility rates $\mathbf{f}$ do not change over time. Under these conditions, the total population size may vary, but the relative proportions between age classes converge to a fixed structure. In this simulation study, the stable age distribution is used as a probability distribution over age classes and is taken to define the parameter vector $\boldsymbol{\pi}$ of the latent distribution introduced in Section~\ref{eq:latent_age_dist}. This stable structure can be obtained from the Leslie matrix $\mathbf{L}$.

In particular, it is determined by the dominant eigenvalue $\lambda_1$ and the corresponding right eigenvector $\mathbf{w}$. The existence of a unique dominant eigenvalue $\lambda_1$, which is real, non-negative, and greater in magnitude than all other eigenvalues $\lambda_i$, is guaranteed for non-negative primitive matrices by the Perron Frobenius theorem \parencite{caswell2001}. If $\mathbf{L}$ is primitive, then as $t \rightarrow \infty$, the population vector $\mathbf{v}_t$ satisfies
\[
\frac{\mathbf{v}_t}{\sum_j n_{j,t}} \to \boldsymbol{w},
\]
where $\boldsymbol{w}$ is the normalized right eigenvector associated with $\lambda_1$, which defines the stable age distribution used in this study, i.e. $\boldsymbol{\pi} = \boldsymbol{w}$.
...
\]
which gives the total population size at time $t+n$ given an initial age distribution $\boldsymbol{x}$. The relative deviation in total population size is then defined as
\begin{equation}
\mathrm{RelErr}_{t+n}
=
\frac{
...
We consider a population structured into $k = 20$ age classes, assuming a maximum age of 20 years due to the negligible frequency of individuals older than 20. Consistent with the discretization used in \cite{nina_bearmodel}. Based on this age structure, we defined a $20 \times 20$ Leslie matrix $\mathbf{L}$ as described in section \ref{sec:pop_model_matrix}. Each age class represents a one-year interval, where the age-class index $j \in \{1,\dots,20\}$ corresponds to the interval $[j-1, j)$ for $j = 1,\dots,19$, while the final class $j=20$ represents individuals aged $19$ years and older.
...
In this section, we define the discretisation procedure used to construct the misclassification matrix $\textbf{M}$ described in Section \ref{par:measurement-error}. The chosen error model is designed to approximately reflect the error pattern shown in Figure 5(a) in \cite{nakamura2023}. An illustrative example is provided in the Appendix (Figure~\ref{fig:error_example_sigma_1}).

The construction is illustrated in the Appendix.

For each individual with true age class $T_r = j$, we assume an additive Gaussian measurement error as specified in \ref{par:measurement-error} with two distinct values of $\sigma$, namely $\sigma \in \{1,4\}$:
\[
...
\begin{figure}[H]
	...
	\caption{
		The stable age distribution $\pi$ and the corresponding observed age distributions $M\pi$ under a measurement error model 
		$\epsilon_r \mid T_r \sim \mathcal{N}(0, \sigma^2)$ with $\sigma \in \{1, 4\}$.
	}
...
This section describes the experimental setup. The latent age distribution $\boldsymbol{\pi}$ is fixed across all simulations and corresponds to \ref{fig:stable_age}. Measurement error is introduced through a misclassification matrix $\boldsymbol{M}$, constructed from the Gaussian error model described in Section~\ref{sec:error_terms} with two choices of $\sigma$, $\sigma = 1$ and $\sigma = 4$.
...
\begin{itemize}
	...
	\item the EM-based maximum likelihood estimator $\hat{\boldsymbol{\pi}}_{ML}$ with naive initialization $\hat{\boldsymbol{\pi}}_{naive}$
\end{itemize}
...
\]
with $\epsilon = 10^{-4}$, or after a maximum of 1000 iterations.The regularization parameter for the MoM estimator, $\lambda$, was chosen by visual inspection of Figure~\ref{fig:median_ci_dist}, selecting the smallest value that still produced a sufficiently smooth estimate. This resulted in $\lambda = 10^{-1}$ for $\sigma = 1$ and $\lambda = 10^{-2}$ for $\sigma = 4$.
...
For each individual with true age class $T_r = j$, the observed class is sampled according to the misclassification probabilities:
\[
O_r \mid T_r = j \sim \text{Categorical}(M_{j\cdot}).
\]
...
In Figure~\ref{fig:tv_vs_sample_size}, we observe how sample size affects the total variation (TV) distance across estimators. The method of moments (MoM) estimator exhibits a slightly higher TV distance across both noise settings relative to the naive estimator, with little sensitivity to sample size. The naive estimator tracks the oracle baseline more closely than the ML and MoM estimators in both noise settings except for $\sigma = 4$ and $n > 500$. 
...
Similarly to the naive estimator the MoM would roughly converge with $n \to \infty$ to:
...
The method-of-moments (MoM) estimator is limited in its recovery of the latent age distribution by the instability of matrix inversion. Matrix inversion does not guarantee constraints such as non-negative probabilities or summing to one, which is accounted for by truncation to 0 and normalization. This projection violates the assumptions of the continuous mapping theorem; thus, we do not expect exact convergence to equation~\eqref{eq:conv_tv_mom}. The MoM is therefore limited in its recovery of the latent age distribution by this bias.

The EM maximum likelihood (ML) estimator, as shown in the results, exhibits a decreasing TV distance with increasing sample size relative to the oracle baseline see Figure~\ref{fig:dif_tv_rel_oracle}. However, it shows a significantly larger spread in its TV distribution, as illustrated in Figure~\ref{fig:violin_TV}. The overall smooth shape of the violin plot, with no clear clustering, suggests that the EM algorithm does not converge to distinct local maxima or plateaus. Instead, it appears to operate on a relatively flat log-likelihood surface, consistent with the convergence diagnostics in Appendix~ \ref{app:failure_rate}, which shows that the maximum number of iterations is rarely reached. This is symptomatic of an ill-posed EM deconvolution problem, where weak identifiability leads to or unstable convergence behaviour \parencites[p.~28]{mclachlan2008}. The results suggest that this instability is partially mitigated as sample size increases.
...
As shown in Figure~\ref{fig:tp_forecast}, both MoM and ML outperform the naive estimator in terms of mean and median under both high and low noise, as well as for small and large sample sizes. In particular, we observe that MoM exhibits a slightly wider Monte Carlo percentile band in Figure~\ref{fig:median_ci_dist}, closer to that of the naive estimator, but with a differently skewed bias.

This suggests that for ecological indicators such as these, the direction and structure of the bias may be more important than its magnitude alone. Specifically we know from the table in Appendix~\ref{app:leslie_params} that misclassifying individuals of fertile $\geq 4$ years to none fertile ages $< 4$ or vice-versa will significantly alter the forecast. This is consistent with our expected measurement error in Figure~\ref{fig:stable_age_vs_D} thus leading to the naive estimator overestimating individuals of fertile age. 
...
In order to study the deconvolution methods in a controlled setting, several simplifying assumptions were made. The misclassification matrix $\boldsymbol{M}$ was assumed to be fully known. In practical applications, however, $\boldsymbol{M}$ must typically be estimated from auxiliary data such as the one observed in panel a shown in Figure~5 of \cite{nakamura2023}, introducing additional uncertainty. The results presented in this study should therefore be interpreted with caution, as they represent a best-case scenario for deconvolution performance.
...
\begin{table}[H]
	\centering
	\input{../tables/failure_rate_ml.tex}
	\caption{
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{../figures/error_model_example.pdf}
	\caption{Illustrative simulation of five individuals per age class $j$ under the measurement error model with $\sigma = 1$. The resulting misclassification pattern is designed to replicate that in Figure~5(a) of \cite{nakamura2023}.}
	\label{fig:error_example_sigma_1}
\end{figure}
